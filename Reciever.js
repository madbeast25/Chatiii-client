...
db.query("SELECT * FROM chats;",(err,res)=>{
        if(err){
          console.log(err);
        }else{
          allchats=res.rows;
...
app.get("/chat",(req,res)=>{
    chats.forEach((chat)=>{
         time=chat.time;
         if(chat.name==receiver){
         lastchat=chat.chat;
         }
    });
    res.render("chat.ejs",{chated:chats,users:contact_records,Time:time,recentchat:lastchat,time_now:currentTime,user_now:appuser,received:receiver,sended:sender});
});

app.listen(Receiver_port,()=>{
  console.log(`Server is running on port ${Receiver_port}`);
});